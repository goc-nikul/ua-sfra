 ...
 * Helpers
 */
var promotionHelper = require('*/cartridge/scripts/helpers/products/promotionHelper');
var productHelper = require('*/cartridge/scripts/helpers/products/productHelper');
...
var getPricebookIDs = require('*/cartridge/scripts/helpers/config/getPricebookIDs');

/**
 * Pre-settings
 */
var promoPricingEnabled = promotionHelper.promoPricingEnabled();
 ...
 * @param {object} Additional data object.
 * @returns {boolean} A boolean.
 */
ProductReader.prototype.shouldAddVariant = function (variant, data) {
  ...
  if (!productHelper.hasValidPrice(variant, data.listPriceBookID, data.salePriceBookID)) {
    return false;
  }
  ...
  if (!productHelper.shouldAddOOSProduct(variant)) {
    return false;
  }
...
ProductReader.prototype.readNextLine = function () {
  ...
  /**
   * Get the pricebooksIDs.
   */
  var sitePricebookIDs = getPricebookIDs();

  /**
   * Get the next product to iterate over.
   */
  data.salePriceBookID = sitePricebookIDs.sale;
  data.listPriceBookID = sitePricebookIDs.list;
  data.promoPricingEnabled = promoPricingEnabled;
  ...
  if (this.currentRecord.variants.length > this.currentRecord.variantIndex) {
    ...
    data.imageData = getImage(currentVariant);

    valid = this.shouldAddVariant(currentVariant, data);
