 ...
 *
 * @param {String} fileName - fileName to generate url
 * @param {Number} expires - Expiration time (in seconds) to be used while generating presigned url
 * @param {String} host - Host override for UACAPI
 * @param {String} bucketName - Bucket Name override for UACAPI
 * @returns {String} presignedUrl - presigned url
 */
S3TransferClient.prototype.getPreSignedUrl = function (fileName, expires, host, bucketName) {
    var presignedUrl;
    var bucketName = bucketName ? bucketName : BUCKETNAME;
    var filePath = '/' + bucketName + '/' + fileName;
    var preSignedCredential = encodeSlash(ACCESSKEY + '/' + CREDENTIALSCOPE);
    var preSignedHost = host ? host : 's3.amazonaws.com';
    var preSignedparameters = StringUtils.format('X-Amz-Algorithm={0}&X-Amz-Credential={1}&X-Amz-Date={2}&X-Amz-Expires={3}&X-Amz-SignedHeaders=host', ALGORITHM, preSignedCredential, DATETIMESTAMP, expires);
    var preSignedCanonicalRequest = getCanonicalRequest('GET', filePath, preSignedparameters, 'host:' + preSignedHost, '\nhost', 'UNSIGNED-PAYLOAD');
    var preSignedStringToSign = getStringToSign(ALGORITHM, DATETIMESTAMP, CREDENTIALSCOPE, preSignedCanonicalRequest);
    var preSignedSignature = Encoding.toHex(sign(SIGNINGKEY, preSignedStringToSign));
    presignedUrl = 'https://' + preSignedHost + filePath + '?' + preSignedparameters + '&X-Amz-Signature=' + preSignedSignature;
    return presignedUrl;
};
