...
function sendTrigger(hookID, promise, data, cb){
    if (promise.isPending()) {
        ...
        if (!empty(data.localeData) && data.localeData.length >= 2) {
            data.SiteLanguage = data.localeData[0].toUpperCase();
            data.SiteCountry = data.localeData[1].toUpperCase();
        }
...
function addDataExtensions(hookID, promise, data, cb) {
	if (promise.isPending()) {

		Logger.debug('MC hook {0} executed', hookID);
        var trigger = require(module.cartridge).trigger(hookID);
        trigger.newMessage(data);
        trigger.dataExtensionMessage(data);

        var result = trigger.sendDataExtension();
...
exports.addDataExtensions = addDataExtensions;