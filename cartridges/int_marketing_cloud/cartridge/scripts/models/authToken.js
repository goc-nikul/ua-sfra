...
const customObjectNameDataEvents = 'MarketingCloudAuthTokenDataEvents'
 ...
 * @returns {dw.object.CustomAttributes} Returns token custom attributes
 */
function getObjectDataEvents() {
    var siteID = require('dw/system/Site').current.ID;
    return helpers.getCustomObject(customObjectNameDataEvents, siteID, true);
}
 ...
 * @returns {Object} Returns the same plain JS object
 */
function updateCachedTokenObjectDataEvents(obj) {
    var custObj = getObjectDataEvents();
    const tokenObject = {	//creating new object with property names matching the rest of the application.
        accessToken: obj.access_token,
        tokenType: obj.token_type,
        expiresIn: obj.expires_in,
 ...
 * @alias module:models/authToken~AuthToken#isValidAuth
 */
function isValidAuthDataEvents() {
    if(!this.token || !this.token.accessToken){
        var cachedToken = getObjectDataEvents();
        if (!cachedToken || !cachedToken.token) {
            return false;
        }
        this.token = JSON.parse(cachedToken.token);
...
function getValidTokenDataEvents() {
    if (!this.isValidAuthDataEvents()){
        var result = require('int_marketing_cloud').restService('authDataEvents').call();
        if (result.status === 'OK' && result.object) {
            this.token = updateCachedTokenObjectDataEvents(result.object);
        }
    }

    return this.isValidAuthDataEvents() && this.token;
}
...
AuthToken.prototype = {
    ...
    isValidAuthDataEvents: function isValidDataEvents(){
        return isValidAuthDataEvents.apply(this);
    },
    ...
    getValidTokenDataEvents: function getValidDataEvents(){
        return getValidTokenDataEvents.apply(this);
    },
    ...
    refreshTokenDataEvents: function() {
        var result = require('int_marketing_cloud').restService('authDataEvents').call();
        if (result.status === 'OK' && result.object) {
            this.token = updateCachedTokenObjectDataEvents(result.object);
        }
