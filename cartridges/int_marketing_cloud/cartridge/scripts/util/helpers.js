...
/**
 * Generate Request XML for TransactionXML.
 * @param {object} attribute Attribute that is missing
 * @param {XML} [message] Optional custom message
 * @returns {XML}
 */
function generateRequestXML(obj, innerXml) {
    for (let field in obj) {
        let val = obj[field]
        if (typeof val === 'number' || typeof val === 'string') {
            innerXml[field] = val;
        } else if (Array.isArray(val)) {
            let subXml = new XML('<' + field + '></' + field + '>'),
                isObjInside = false;
            for (var i = 0; i < val.length; i++) {
                let key = Object.keys(val[i])[0];
                if (typeof val[i][key] === 'object') {
                    isObjInside = true;
                    generateRequestXML(val[i], subXml);
                } else {
                    if (Array.isArray(val)){
                        subXml = new XML('<' + field + '></' + field + '>');
                    }
                    innerXml.appendChild(generateRequestXML(val[i], subXml));
                }
            }

            if (isObjInside) {
                innerXml.appendChild(subXml);
            }
        } else if (typeof val === 'object') {
            let subXml = new XML('<' + field + '></' + field + '>');
            innerXml.appendChild(generateRequestXML(val, subXml));
        }
    }
    return innerXml;
}
...
exports.generateRequestXML = generateRequestXML;
