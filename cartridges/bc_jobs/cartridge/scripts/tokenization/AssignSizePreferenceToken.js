...
function generateProductTokenizationXML(params) {
    ...
    try {
        ...
        let products = CatalogMgr.getCategory('prep-category').getProducts().iterator();
        while (products.hasNext()) {
            ...
            //master product
            writeProductXML(xsw, product, 'masterSizePrefJSON');
            
           //Get all variants
            let variants = product.getVariants().iterator();
            while (variants.hasNext()) {
                let variant = variants.next();
                writeProductXML(xsw, variant, 'variationSizePrefJSON');
            }
        }
        /**
...
function writeProductXML(xsWriter, product, sizePrefJSON) {
    ...
    xsWriter.writeAttribute('attribute-id', sizePrefJSON);
    var sizePrefToken = product.master ? createMasterSizePrefToken(product) : createVariationSizePrefToken(product);
    xsWriter.writeCharacters(sizePrefToken);
