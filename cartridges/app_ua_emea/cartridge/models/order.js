...
function updateItemInfo(lineItemContainer, options, orderItems, requestLocale) {
    ...
    collections.forEach(productLineItems, function (productLineItem) {
        ...
        if (options && (options.containerView === 'orderDetails')) {
            ...
            item.shippedTo = (shippingAddress.firstName ? (shippingAddress.firstName + ' ') : '') + (shippingAddress.lastName ? shippingAddress.lastName : null);
...
function OrderModel(lineItemContainer, options) {
    ...
    if (options && options.containerView === 'basket') {
        ...
        var COHelpers = require('*/cartridge/scripts/checkout/checkoutHelpers'); // eslint-disable-next-line
        var siteCountryCode = Locale.getLocale(request.locale).country;
        var internationalShippingCountriesList = COHelpers.getInternationalShippingCountriesList(siteCountryCode);
        if (internationalShippingCountriesList && internationalShippingCountriesList.length > 0) {
            this.internationalShippingCountriesList = internationalShippingCountriesList;
        }
            ...
            if ('refundsJson' in lineItemContainer.custom && !empty(lineItemContainer.custom.refundsJson) && (this.status === 'SHIPPED' || this.status === 'PARTIAL_SHIPPED')) {
                ...
                if (refunds && refunds.length > 0) {
                    ...
                    if (recentRefundDate) {
                        ...
                        if (Site.getCurrent().getID() === 'KR') {
                            this.updatedDate = StringUtils.formatCalendar(new Calendar(recentRefundDate), 'MM/dd');
                        } else {
                ...
                if (shippedData && shippedData.length > 0) {
                    ...
                    if (recentShippedDate) {
                        ...
                        if (Site.getCurrent().getID() === 'KR') {
                            this.updatedDate = StringUtils.formatCalendar(new Calendar(recentShippedDate), 'MM/dd');
                        } else {
        ...
        this.hasMultipleShipments = hasMultipleShipments;
