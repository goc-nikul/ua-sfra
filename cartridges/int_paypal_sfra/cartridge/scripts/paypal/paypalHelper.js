    ...
    for (var i = 0, len = lineItems.size(); i < len; i++) {
        ...
        if (productLineItem instanceof dw.order.ProductLineItem || productLineItem instanceof dw.catalog.Variant) {
            ...
            if (product.isVariant()) {
            	var productStyle = !empty(product.custom.style) ? product.custom.style + '-' : '';
            	var productColor = !empty(product.custom.color) ? product.custom.color + '-' : '';
            	var productSize =  !empty(product.custom.size) ? product.custom.size : '';
            	productCustomNumber = productStyle + productColor + productSize;
            } else {
            	productCustomNumber = product.getID();
            }
            ...
            data.put(prefix + 'NUMBER' + index, productCustomNumber);
...
paypalHelper.removeAllPaypalPaymentInstruments = function (basket) {
    ...
    var iterator = paymentInstruments.iterator();
    var paymentInstrument = null;
    // Remove all Payment Instruments before creating the new one.
    // In case if the basket contains Credit card as payment instrument,
    // We should be removing that as well, because we are not supporting split payment with Credit card.
...
/**
 * validateShippingAddress() validate shipping address
 *
 * @param {dw.order.OrderAddress} shippingAddress - Current shipping address
 * @returns {boolean} shippingAddressFulfilled - valid shipping address
 */
paypalHelper.validateShippingAddress = function(shippingAddress) {
    var shippingAddressFulfilled = true;

    //validate the Non ISO Latin-1 in the paypal address
    var regXLatin = /[^\u0020-\u007F\u00A0-\u024F\u1E00-\u1EFF]/g;
    var addressStr = shippingAddress.firstName + shippingAddress.lastName + shippingAddress.address1 + shippingAddress.address2 + shippingAddress.city;
    var isNonLatinCharExist = regXLatin.test(addressStr);
    // verify the country of selected paypal address and webstore.
    var locale = request.getLocale();
    locale = (locale === 'default') ? 'en_US' : locale;

    // Check that the ship to country is Valid
    var countryLocale = require('dw/util/Locale').getLocale(locale);
    var webStoreCountry = countryLocale.country;
    var isCountryValid = shippingAddress.countryCode && shippingAddress.countryCode.value && webStoreCountry ? 
        (shippingAddress.countryCode.value).toUpperCase() === webStoreCountry.toUpperCase() : false;

    if (
        (empty(shippingAddress.firstName)) ||
        (empty(shippingAddress.lastName)) ||
        (empty(shippingAddress.address1)) ||
        (empty(shippingAddress.postalCode)) ||
        (empty(shippingAddress.stateCode)) ||
        (isNonLatinCharExist) ||
        (!isCountryValid)) {
        shippingAddressFulfilled = false;
    }
    return shippingAddressFulfilled;
};

/**
 * validateBillingAddress() validate billing address
 *
 * @param {dw.order.OrderAddress} billingAddress - Current billing address
 * @returns {boolean} billingAddressFulfilled - valid billing address
 */
paypalHelper.validateBillingAddress = function(billingAddress) {
    var billingAddressFulfilled = true;

    //validate the Non ISO Latin-1 in the paypal address
    var regXLatin = /[^\u0020-\u007F\u00A0-\u024F\u1E00-\u1EFF]/g;
    var addressStr = billingAddress.firstName + billingAddress.lastName + billingAddress.address1 + billingAddress.address2 + billingAddress.city + billingAddress.stateCode;
    var isNonLatinCharExist = regXLatin.test(addressStr);

    //Check if state code is empty for billing address
    var emptyState = false;
    if (empty(billingAddress.stateCode) && 
        billingAddress.countryCode && 
        (billingAddress.countryCode.value == 'US' || billingAddress.countryCode.value == 'CA')) {
        emptyState = true;
    }

    if (empty(billingAddress.firstName) ||
        (empty(billingAddress.lastName)) ||
        (empty(billingAddress.address1)) ||
        (empty(billingAddress.postalCode)) ||
        (emptyState) ||
        (isNonLatinCharExist)) {
        billingAddressFulfilled = false;
    }
    return billingAddressFulfilled;
...
paypalHelper.prepareBasketForCheckoutFromCart = function (basket) {
    ...
    if (defaultShipment &&
        ...
        defaultShipment.getShippingAddress().getCountryCode().getValue() && 
        defaultShipment.getShippingAddress().getCountryCode().getValue().toUpperCase() !== webStoreCountry) {
        // If Shipping address Country code doesn't match with the current locale country code 
        // then clear the basket Shipping adress and also from the Shipping Form.
        var shippingForm = require('server').forms.getForm('shipping');
        shippingForm.clear();
        Transaction.wrap(function () {
            defaultShipment.createShippingAddress();
        });
...
/**
 * validateShippingAddressLength() validates shipping address length
 *
 * @param {dw.order.OrderAddress} shippingAddress - Current shipping address
 * @returns {boolean} - true, if shipping address is exceeding 35 characters
 */
paypalHelper.validateShippingAddressLength = function (shippingAddress) {
    if (shippingAddress.address1 !== null) {
        if (shippingAddress.address1.length > 35) {
            return true;
        }
    }
    if (shippingAddress.address2 !== null) {
        if (shippingAddress.address2.length > 35) {
            return true;
        }
    }
    return false;
