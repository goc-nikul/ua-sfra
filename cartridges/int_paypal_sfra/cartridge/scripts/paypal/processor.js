...
var PayPalLogger = require('dw/system/Logger').getLogger('PayPalDoExpressCheckout', 'PayPalDoExpressCheckout');
...
function authorize(order, orderNo, inpPaymentInstrument) {
    ...
    if (!('paypalToken' in paymentInstrument.custom) || empty(paymentInstrument.custom.paypalToken)) {
        PayPalLogger.error('Missing PayPal token. Order ID: {0}', orderNo);
        return { error: true };
    }
    if (!('paypalPayerID' in paymentInstrument.custom) || empty(paymentInstrument.custom.paypalPayerID)) {
        PayPalLogger.error('Missing PayPal PayerID. PayPal Token: {0}. Order ID: {1}', paymentInstrument.custom.paypalToken, orderNo);
        return { error: true };
    }
    ...
    if (doExpressCheckoutPaymentResult.error) {
        ...
        if (doExpressCheckoutPaymentResult.responseData.l_errorcode0 === '10486') {
            ...
            PayPalLogger.warn('Duplicate Request. A successful transaction has already been completed for this token.', paypalErrorMessage);
            return {
                error: true,
                paypalErrorMessage: paypalErrorMessage,
                errorCode: doExpressCheckoutPaymentResult.responseData.l_errorcode0,
