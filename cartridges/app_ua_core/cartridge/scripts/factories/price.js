...
/**
 * Get actual list price for a product
 *
 * @param {dw.catalog.ProductPriceModel} priceModel - Product price model
 * @return {dw.value.Money} - Actual list price
 */
function getActualListPrice(priceModel) {
    ...
    price = priceModel.price.available ? priceModel.price : null;
...
function getPrice(inputProduct, currency, useSimplePrice, promotions, currentOptionModel) {
    ...
    actualListPrice = getActualListPrice(priceModel);
    salesPrice = priceModel.price;
    priceBookSalesPrice = salesPrice;
    ...
    return new DefaultPrice(salesPrice, listPrice, priceBookSalesPrice, actualListPrice);
}

module.exports = {
    getPrice: getPrice,
    getListPrice: getListPrice,
    getActualListPrice: getActualListPrice
};
