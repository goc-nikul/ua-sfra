...
function createCategorySearchRefinement(productSearch, refinementDefinition, Model) {
    ...
    if (currentCategory.root || currentCategory.parent.root) {
        ...
        if (!currentCategory.parent.root && !currentCategory.parent.parent.root) {
            var parentCategoriesIterator = currentCategory.parent.onlineSubCategories.iterator();
            var index = 0;
            while (parentCategoriesIterator.hasNext()) {
                var category = parentCategoriesIterator.next();
                if (category.online && category.hasOnlineProducts()) {
                    childCategory = new Model(productSearch, refinementDefinition, category, category.ID === currentCategory.ID);
                    topCategory.subCategories.push(childCategory);
                    selectedCategoryIndex = (category.ID === currentCategory.ID) ? index : selectedCategoryIndex;
                    index++;
                }
            }
        } else {
        ...
        insertPoint = topCategory.subCategories[selectedCategoryIndex].subCategories;
    }
    // eslint-disable-next-line no-shadow
    collections.forEach(currentCategory.subCategories, function (category) {
        if (category.online && ((category.parent && category.parent.root) || category.hasOnlineProducts())) {
