...
var deliveryDateHelper = require('app_ua_core/cartridge/scripts/util/DeliveryHelper');
...
exports.calculate = function (basket) {
    ...
    if ('verifiedIdmeScope' in basket.custom && !empty(basket.custom.verifiedIdmeScope)) {
    ...
    if (HookMgr.hasHook('dw.order.calculatePaazlShipping')) {
        HookMgr.callHook('dw.order.calculatePaazlShipping', 'calculateShipping', basket);
    } else {
    ...
    // Provider Service Tax Calculation
    if (HookMgr.hasHook('app.tax.calculateTax')) {
        HookMgr.callHook('app.tax.calculateTax', 'calculateTax', basket);
    } else {
    ...
    // ===================================================
    if (HookMgr.hasHook('dw.order.calculateProratedDiscount')) {
        HookMgr.callHook('dw.order.calculateProratedDiscount', 'calculateProratedDiscount', basket);
    }
    ...
    // =====            DONE                         =====
    // ===================================================
	var currentCustomer = basket.customer;
	if (Site.getCurrent().getCustomPreferenceValue('enableVIPCheckoutExperience') && currentCustomer.authenticated && currentCustomer.registered && !session.custom.suppressVipPointsCheck) {
		const vipDataHelpers = require('*/cartridge/scripts/vipDataHelpers');
		vipDataHelpers.handleVIPPayment(currentCustomer, basket);
    }
    // Recalculate gift card amount when exists
...
function calculateProductPrices (basket) {
    ...
    while (productLineItems.hasNext()) {
        ...
        if (!productLineItem.catalogProduct || (product && product.custom.giftCard.value == 'EGIFT_CARD')) {
...
exports.calculateShipping = function(basket) {
    ...
    shippingHelpers.setShippingMethodPrice(basket);
    return new Status(Status.OK);
}

exports.setDeliveryEstimateDate = function(basket) {
    // Setting delivery date at the shipment level
	collections.forEach(basket.getShipments(), function(shipment){
		if (shipment && shipment.ID !== 'EGiftCardShipment' && shipment.shippingMethodID !== 'eGift_Card') {
			var shipMethod = shipment.getShippingMethod();
		    var deliveryDateRange = deliveryDateHelper.getShippingDeliveryDates(shipMethod, false);
		     if (deliveryDateRange) {
		    	 shipment.custom.deliveryDateMin = deliveryDateRange[0].getTime();
		    	 shipment.custom.deliveryDateMax = deliveryDateRange[1].getTime();
		     }
		} else {
			var egcProduct = shipment && shipment.productLineItems.length > 0 ? shipment.productLineItems[0] : null;
			if (egcProduct) {
				shipment.custom.deliveryDateMin = egcProduct.custom.gcDeliveryDate;
		    	shipment.custom.deliveryDateMax = egcProduct.custom.gcDeliveryDate;
			}
		}
	});
