...
function validateBasket(basket, validateTax) {
    ...
    if (!basket) {
        ...
        if (productExistence.error && productExistence.hasEGC && !productExistence.isEGCValid) {
            result.error = true;
            result.message = Resource.msg('error.cart.or.checkout.error.egc', 'cart', null);
        } else if (productExistence.error || !productExistence.hasInventory) {
            result.error = true;
