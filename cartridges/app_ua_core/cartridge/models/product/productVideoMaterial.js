...
function getSelectedColor(productObject, colorObject) {
    ...
    if (product === null) { // Not possible to reach here because it would have thrown error before reaching this
        return null;
    }
    const variationModel = product.variationModel;
    const currentVariationModel = product.isVariant() ? product.masterProduct.variationModel : product.variationModel;
    ...
    if (!selectedColor) {
        selectedColor = !empty(colorObject) && !empty(colorObject.color) ? colorObject.color : selectedColor;
    }
    ...
    if (selectedColor) {
        ...
        if (!currentVariationModel.hasOrderableVariants(colorVA, cv)) {
            for (let i = 0, il = variationModel.variants.length; i < il; i++) {
                cv = currentVariationModel.getVariationValue(variationModel.variants[i], colorVA);
 ...
 * @param {dw.catalog.Product} colorObject - selected color object
 */
module.exports = function ProductVideoMaterial(productObject, colorObject) {
    const product = productObject;
    let video360Material = [];
    try {
        const masterID = product.isVariant() ? product.masterProduct.ID : product.ID;
        const selectedColor = getSelectedColor(product, colorObject);
        if (product.custom.division === 'Footwear') {
            video360Material.push({
                poster_url: 'https://underarmour.scene7.com/is/image/Underarmour/' + masterID + '-' + selectedColor.value + '_DEFAULT?bgc=f0f0f0&wid=640&hei=480&size=550,460',
                video_url_mp4: 'https://underarmour.scene7.com/is/content/Underarmour/auto_dim7_' + masterID + '-' + selectedColor.value + '-0x480-300k',
                masterID_selectedColor: masterID + '-' + selectedColor.value
            });
        }
