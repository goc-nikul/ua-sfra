...
function renderNewPageOfItems(pageNumber, spinner, focusElementSelector) {
    ...
    var sortRule = $('.wishlistItemCardsData').data('sort-rule');
    if (spinner) {
    ...
    $.ajax({
        ...
        $('.wishlistItemCards').remove();
        // If not defined or null or the trim of the length = 0
        // Then remove the sorting dropdown box.
        // Appears that data is return a carriage return.
        if (!data || data.trim().length === 0) {
            $('.wishlist-sort').remove();
        }
 ...
 * @param {string} element - selector of the element
 * @return {Object} - Object with properties top, left, right and bottom.
 */
...
module.exports = {
    ...
    moreWLItemsOnScroll: function () {
        $(window).on('scroll', function () {
            if ($('.js-grid_footer button.more-wl-items').length) {
                if (isInViewport('.js-grid_footer')) {
                    if (isLoading) {
                        isLoading = false;
                        $('body .js-grid_footer button.more-wl-items').trigger('click');
                    }
                }
            }
        });
    ...
    sortWishlist: function () {
        $('#wishlistItemCards').on('change', '.js-sort-wishlist-select', function (e) {
            e.preventDefault();

            $.spinner().start();
            $(this).trigger('wishlist:sortWishlist', this.value);

            var publicView = $('.wishlistItemCardsData').data('public-view');
            var listUUID = $('.wishlistItemCardsData').data('uuid');
            var url = $(this).val();
            var sortRule = $(this).find('option:selected').data('id');
            var pageNumber = $('.wishlistItemCardsData').data('page-number');
            ...
            $.ajax({
                ...
                $('.wishlistItemCards').remove();
                $('body #wishlistItemCards').append(data);
                $('body #wishlistItemCards').find('.wishlistItemCardsData').attr('data-sort-rule', sortRule);
                $.spinner().stop();
            }).fail(function () {
                $.spinner().stop();
            });
