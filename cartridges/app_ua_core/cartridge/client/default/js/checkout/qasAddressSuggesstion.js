...
function addressSuggestion(options) { // eslint-disable-line
            ...
            address: {
                ...
                search: function (instance) {
                    ...
                    if (instance.elements.location) {
                        url += '&location=' + $('#geoLocationDetails').attr('value');
                    }
    ...
    ContactDataServices.address = function (customOptions) {
        ...
        instance.canSearch = function () {
            ...
            if (currentCountryCode === 'US' && instance.currentSearchTerm.length > 1) {
                instance.currentCountryCode = 'USA';
                validCountry = true;
            } else if (currentCountryCode === 'CA' && instance.currentSearchTerm.length > 1) {
                instance.currentCountryCode = 'CAN';
                validCountry = true;
            }
            ...
            show: function (items) {
                ...
                if (instance.picklist.size > 0) {
                    ...
                    enterManuallyOption.addEventListener('click', function () {
                        ...
                        var position = element && element.length ? (element.offset().top - $('.l-header-section_bottom').outerHeight() - ($('.b-checkout_sticky-applypromo').length > 0 ? $('.b-checkout_sticky-applypromo').outerHeight() : 0)) : 0;
        ...
        instance.result = {
            ...
            show: function (data) {
                ...
                if (data.address && data.address.length > 0) {
                    ...
                    if ($('#billingCountry').val() !== '') {
                        $('.b-checkout_inner-nextStep .submit-payment').attr('disabled', false);
                    }
            ...
            updateAddressLine: function (key, addressLineObject, className) {
                ...
                if (instance.elements[key]) {
                    ...
                    if ($('#billingState').val() !== '') {
                        $('#billingState').closest('.form-group').removeClass('error-field');
