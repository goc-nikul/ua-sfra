import FormGenericCore from 'org/components/forms/FormGeneric';
import clientSideValidation from '../common/clientSideValidation';

export default class FormNA extends FormGenericCore {
    /**
    ...
    handleHackEntries(event) {
        ...
        $(this.$el).find('input:visible, select:visible').each((index, input) => {
            ...
            if (customRegexExp.test(str) || (regex.test(str) || caretRegex.test(str)) || (isAddressField && addressRegex.test(str)) || (isCityField && !cityRegex.test(str) && str !== '')) {
                ...
                $this.addClass('is-invalid');
                $('html, body').animate({
                    scrollTop: $this.parents('.form-group').offset().top - $('.js-header').height() - 10
                }, 500);
            } else {
                $this.closest('.form-group').removeClass('error-field');
                $this.removeClass('is-invalid');
