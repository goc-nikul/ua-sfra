...
function writeProductFeedItem(item, localeMap) {
	...
	switch(item.type) {
		case 'Brands':
			...
			writeElementCDATA('Name', "Under Armour");
	        writeElementCDATA('ExternalId', "UA");
			_xmlStreamWriter.writeEndElement();
			break;
		...
		case 'Products':
			...
			var isOnlineOrEcommAssortment = product.online || (product.custom && 'ecommAssortment' in product.custom && product.custom.ecommAssortment);
			if (isOnlineOrEcommAssortment && product.searchable && product.master) {
			    ...
			    writeElementCDATA('BrandExternalId', "UA");
			    ...
			    if(product.manufacturerSKU != null){
			        ...
			        _xmlStreamWriter.writeEndElement();
				    	...
				    	for(var i = 0; i < product.variants.length; i++) {
				            if(!empty(product.variants[i].custom.sku)) {
				            	showSkus = true;
				                break;
				            }
				        }
				        if(showSkus) {
				        	...
				        	for(var i = 0; i < product.variants.length; i++) {
					            if(!empty(product.variants[i].custom.sku)) {
					                writeElement('ManufacturerPartNumber', product.variants[i].custom.sku);
					            }
					        }
					        _xmlStreamWriter.writeEndElement();
				        }
			    ...
			    if(BV_Constants.EnableProductFamilies && (product.master || product.variant)) {
			    	...
			    	var gender = product.master ? product.custom.gender : product.variationModel.master.custom.gender;
			    	
			    	_xmlStreamWriter.writeStartElement('Attribute');
			    	_xmlStreamWriter.writeAttribute('id', 'Gender');
			    	writeElement('Value', gender);
			    	_xmlStreamWriter.writeEndElement();
	    	
			    	//write End use attribute
			    	var enduse = product.master ? product.custom.enduse : product.variationModel.master.custom.enduse;
			    	
			    	_xmlStreamWriter.writeStartElement('Attribute');
			    	_xmlStreamWriter.writeAttribute('id', 'Primary_Sport');
			    	writeElement('Value', enduse);
			    	_xmlStreamWriter.writeEndElement();
			    	
			    	
			    	//write Category attribute
			    	var silhouette = product.master ? product.custom.silhouette : product.variationModel.master.custom.silhouette;
			    	
			    	_xmlStreamWriter.writeStartElement('Attribute');
			    	_xmlStreamWriter.writeAttribute('id', 'Category');
			    	writeElement('Value', silhouette);
			    	_xmlStreamWriter.writeEndElement();
			    	
			    	//write Sub Category attribute
			    	var subsilhoutte = product.master ? product.custom.subsilhouette : product.variationModel.master.custom.subsilhouette;
			    	
			    	_xmlStreamWriter.writeStartElement('Attribute');
			    	_xmlStreamWriter.writeAttribute('id', 'Sub_Category');
			    	writeElement('Value', subsilhoutte);
			    	_xmlStreamWriter.writeEndElement();
