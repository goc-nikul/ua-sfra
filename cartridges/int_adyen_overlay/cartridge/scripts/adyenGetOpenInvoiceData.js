...
function getLineItems(_ref) {
  ...
  for (var item in allLineItems) {
    ...
    if (lineItem instanceof dw.order.ProductLineItem && lineItem.product && !lineItem.bonusProductLineItem || lineItem instanceof dw.order.ShippingLineItem || lineItem instanceof dw.order.PriceAdjustment && lineItem.promotion.promotionClass === dw.campaign.Promotion.PROMOTION_CLASS_ORDER) {
      var lineItemObject = {};
      ...
      var vatAmount = LineItemHelper.getVatAmount(lineItem) ? LineItemHelper.getVatAmount(lineItem).divide(quantity) : null;
      var vatPercentage = LineItemHelper.getVatPercentage(lineItem);
      lineItemObject.amountExcludingTax = itemAmount.getValue().toFixed();
      lineItemObject.taxAmount = vatAmount ? vatAmount.getValue().toFixed() : 0;
