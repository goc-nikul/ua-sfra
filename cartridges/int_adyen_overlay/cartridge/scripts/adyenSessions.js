...
function createSession(basket, customer, countryCode) {
  try {
    ...
    if (basket) {
      if (basket.totalGrossPrice.currencyCode === 'N/A') {
        var error = new Error('Basket currencyCode is N/A');
        error.noCurrencyCode = true;
        throw error;
      }
    ...
    if (PreferencesUtil.isCountryEnabled('afterPayEnabled')) {
      var COHelpers = require('*/cartridge/scripts/checkout/checkoutHelpers');
      if (COHelpers.isBasketExceedingAfterPayLimit(basket)) {
        if (!sessionsRequest.blockedPaymentMethods) {
          sessionsRequest.blockedPaymentMethods = [];
        }
        sessionsRequest.blockedPaymentMethods.push('afterpay');
      }
    }
    ...
    if (e.noCurrencyCode) {
      // Drop down error log level from fatal if the reason is missing currencyCode
