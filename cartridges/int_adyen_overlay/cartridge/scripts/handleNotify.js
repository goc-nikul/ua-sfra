...
function notify(notificationData) {
  ...
  var orderNumberPrefix = Site.getCurrent().getCustomPreferenceValue('OrderNumberPrefix');
  ...
  // SKIP the UKIE orders in EU site and vice-versa
  if (!notificationData.merchantReference || notificationData.merchantReference.indexOf(orderNumberPrefix) === -1) {
    return {
      success: true,
