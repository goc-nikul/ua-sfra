...
var adyenConfigsObj = {
  ...
  getAdyenMerchantAccount: function getAdyenMerchantAccount(currencyCode) {
    try {
      if (currencyCode) {
        return JSON.parse(getCustomPreference('Adyen_merchantCode'))[currencyCode];
      } else {
        return JSON.parse(getCustomPreference('Adyen_merchantCode'))[session.currency.currencyCode];
      }
    } catch (e) {
      return '';
    }
