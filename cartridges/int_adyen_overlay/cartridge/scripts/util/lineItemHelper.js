...
var __LineItemHelper = {
  getDescription: function getDescription(lineItem) {
    ...
    if (lineItem instanceof dw.order.ProductLineItem && lineItem.product) {
  ...
  getVatAmount: function getVatAmount(lineItem) {
    if (lineItem instanceof dw.order.ProductLineItem && lineItem.product || lineItem instanceof dw.order.ShippingLineItem) {
  ...
  getItemAmount: function getItemAmount(lineItem) {
    if (lineItem instanceof dw.order.ProductLineItem && lineItem.product || lineItem instanceof dw.order.ShippingLineItem) {
      return AdyenHelper.getCurrencyValueForApi(lineItem.taxBasis.subtract(lineItem.getAdjustedTax()));
    }
    if (lineItem instanceof dw.order.PriceAdjustment) {
