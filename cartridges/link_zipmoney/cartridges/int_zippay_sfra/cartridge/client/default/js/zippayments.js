...
ZipPaymentsSubmitPaymentStage.prototype.initStage = function () {
    ...
    var zipResources = window.ZipResources;
    var errorValue = zipResources.error;
    ...
    if (ZipResources.hasError() && !this.zipPayments.isErrorSeen()) {
        if (errorValue === '2') {
            ...
            this.zipPayments.showError({
                message: ZipResources.getText('zipReferralOrderError'),
                scroll: true
            });
...
ZipPaymentsPlaceOrderStage.prototype.refreshStage = function () {
    ...
    if (this.zipPayments.isZipPaymentMethod(selectedPaymentMethod) && !this.zipPayments.getHasZipToken()) {
        ...
        $('.zip-checkout').html(ZipResources.getText('continueTo' + this.zipPayments.getSelectedPaymentMethod()));
    } else {
...
ZipPaymentsPlaceOrderStage.prototype.initPlaceOrderButton = function () {
    ...
    $continueToZipBtn.html(ZipResources.getText('continueTo' + this.zipPayments.getSelectedPaymentMethod()));
        ...
        $continueToZipBtn.click(function (ev) {
            ...
            if (!form.find('input.is-invalid').length) {
                $.ajax({
                    ...
                    data: $('.b-contact-info form').serialize(),
                    method: 'POST',
                    success: function (data) {
...
ZipPaymentsPlaceOrderStage.prototype.handleZipError = function (errorCode) {
    ...
    switch (errorCode) {
        ...
        case 'shopper.billing_address.country':
            errorMsg = ZipResources.getText('zipInvalidCountryError');
            break;
        case 'Invalid Country':
            errorMsg = ZipResources.getText('zipInvalidCountryError');
            break;
        case 'Invalid Phone':
            errorMsg = ZipResources.getText('zipInvalidPhoneNumber');
            break;
        default:
...
ZipPayments.prototype.init = function () {
    var zipResourceJSON = $('.zip-resource').attr('data-zip-resource');
    window.ZipResources = JSON.parse(zipResourceJSON);
};
