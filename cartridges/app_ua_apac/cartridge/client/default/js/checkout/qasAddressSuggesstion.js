/* *
 * addres suggestion for shipping and billing
  */
'use strict';

/* *
 * addres suggestion for shipping and billing
 * @param {Object} options to the suggestion
  */
function addressSuggestion(options) { // eslint-disable-line
    ...
    ContactDataServices.defaults = {
        ...
        useAddressEnteredText: '<div class="hide">Enter address manually</div>',
        useSpinner: false,
        language: 'en',
        addressLineLabels: [
            'addressLine1',
    ...
    ContactDataServices.urls = {
        ...
        getToken: function (instance) {
            if (!instance.token) {
                instance.token = ContactDataServices.urls.getParameter('token'); // eslint-disable-line
            }
        },
    ...
    ContactDataServices.address = function (customOptions) {
        ...
        instance.events = new ContactDataServices.eventFactory(); //  eslint-disable-line
        ...
        instance.validCountries = window.sitePreferences.qasValidCountries;
        ...
        instance.search = function (event) {
            ...
            var currentCountryInfo = instance.countryCodeMapping[instance.countryList.value] || instance.countryList;
        ...
        instance.canSearch = function () {
            ...
            var currentCountryCode = $(instance.input.closest('form')).find('#billingCountry').val();
            if (instance.validCountries[currentCountryCode] && instance.currentSearchTerm.length > 1) {
                instance.currentCountryCode = instance.validCountries[currentCountryCode];
                validCountry = true;
            }
            ...
            return (instance.enabled &&
                ...
                instance.countryList !== undefined && instance.countryList.value !== '' && validCountry &&
        ...
        instance.checkTab = function (event) {
            ...
            e = e.which || e.keyCode;
            if (e === 9 /* Tab */) {
                $('.address-picklist-container').remove(); // instance.input.parentNode.removeChild(instance.picklist.container);
                instance.picklist.keyup(e);
                return;
            }
        };
        ...
        instance.picklist = {
            ...
            show: function (items) {
                ...
                if (instance.picklist.size > 0) {
                    ...
                    instance.picklist.items.forEach(function (item) {
                        ...
                        item.suggestion = item && item.suggestion ? item.suggestion.replace(/\w\S*/g, m => m.charAt(0).toUpperCase() + m.substr(1).toLowerCase()) : '';
                    ...
                    var enterManuallyOption = document.createElement('div');
                    enterManuallyOption.innerHTML = '<b>' + $('#checkout-main').data('enter-manually') + '</b>';
                    instance.picklist.list.appendChild(enterManuallyOption);
                    enterManuallyOption.addEventListener('click', function () {
                        instance.picklist.hide();
                    });
                ...
                $('.address-picklist div').on('click', function () {
                    //  Hide any previous results
                    instance.result.hide();
                    instance.picklist.currentItem = this;
                    instance.picklist.pick(instance.picklist.currentItem);
                });

                $(document).on('click', function (event) {
                    if (!$(event.target).closest('.addressOne').length) {
                        $('.address-picklist-container').remove(); // instance.input.parentNode.removeChild(instance.picklist.container);
                    }
                });
            },
            ...
            createList: function () {
                ...
                list.setAttribute('data-analytics-typeahead-select', '');
                return list;
            },
            ...
            keyup: function (e) {
                ...
                if (e === 13 /* Enter */) {
                    //  Hide any previous results
                    instance.result.hide();
                    var $this = document.querySelector('.address-picklist .selected').outerHTML;
                    var temp = document.createElement('div');
                    temp.innerHTML = $this;
                    instance.picklist.currentItem = temp.firstChild;
                    instance.picklist.pick(instance.picklist.currentItem);
                }

                //  Get a list of all the addresses in the picklist
                var addresses = instance.picklist.list.querySelectorAll('div'), // eslint-disable-line
                    firstAddress, lastAddress; // eslint-disable-line
            ...
            addMatchingEmphasis: function (item) {
                var highlights = item.matched || [], // eslint-disable-line
        ...
        instance.result = {
            ...
            show: function (data) {
                ...
                var businessTypeAddress = false;
                var businessTypeAddressEnabled = window.sitePreferences.businessTypeAddressEnabled;
                if (businessTypeAddressEnabled && businessTypeAddressEnabled.includes(instance.currentCountryCode) && data.metadata && data.metadata.addressClassification && data.metadata.addressClassification.deliveryType && data.metadata.addressClassification.deliveryType === 'business') {
                    businessTypeAddress = true;
                }

                if (data.address && data.address.length > 0) {
                    //  Create an array to hold the hidden input fields
                    var inputArray = []; // eslint-disable-line
                    ...
                    for (var i = 0; i < data.address.length; i++) {
                        ...
                        if (businessTypeAddress) {
                            if (addressComponent.addressLine1) {
                                addressComponent.businessName = addressComponent.addressLine1.replace(/\w\S*/g, m => m.charAt(0).toUpperCase() + m.substr(1).toLowerCase());
                            } else if (addressComponent.addressLine2) {
                                addressComponent.addressLine1 = addressComponent.addressLine2.replace(/\w\S*/g, m => m.charAt(0).toUpperCase() + m.substr(1).toLowerCase());
                                addressComponent.addressLine2 = '';
                            } else if (addressComponent.addressLine3) {
                                addressComponent.addressLine2 = addressComponent.addressLine3.replace(/\w\S*/g, m => m.charAt(0).toUpperCase() + m.substr(1).toLowerCase());
                            }
                        } else if (!businessTypeAddress) {
                            if (addressComponent.addressLine1) {
                                addressComponent.addressLine1 = addressComponent.addressLine1.replace(/\w\S*/g, m => m.charAt(0).toUpperCase() + m.substr(1).toLowerCase());
                            } else if (addressComponent.addressLine2) {
                                addressComponent.addressLine2 = addressComponent.addressLine2.replace(/\w\S*/g, m => m.charAt(0).toUpperCase() + m.substr(1).toLowerCase());
                            }
                        }
                        if (addressComponent.locality) {
                            addressComponent.locality = addressComponent.locality.replace(/\w\S*/g, m => m.charAt(0).toUpperCase() + m.substr(1).toLowerCase());
                            addressComponent.suburb = addressComponent.locality.replace(/\w\S*/g, m => m.charAt(0).toUpperCase() + m.substr(1).toLowerCase());
                        } else if ((addressComponent.province && !!window.sitePreferences.qasProvinceMapping && !!window.sitePreferences.qasProvinceMapping[instance.currentCountryCode])) {
                            var stateName = addressComponent.province.replace(/\w\S*/g, m => m.charAt(0).toUpperCase() + m.substr(1).toLowerCase());
                            addressComponent.province = window.sitePreferences.qasProvinceMapping[instance.currentCountryCode][stateName];
                        }
                        //  The addressComponent object will only have one property, but we don't know the key
                        for (var key in addressComponent) { // eslint-disable-line
                            if (addressComponent.hasOwnProperty(key)) { // eslint-disable-line
                    ...
                    if ($('#billingCountry').val() !== '') {
                        ...
                        var paymentMethod = (paymentMethodsList && paymentMethodsList.find('input[name="brandCode"]:checked')) ? paymentMethodsList.find('input[name="brandCode"]:checked').val() : '';
                        $('.b-checkout_inner-nextStep .submit-payment').attr('disabled', paymentMethod === 'paypal');
                    }
                    ...
                    for (var element in instance.elements) { // eslint-disable-line
                        if (instance.elements.hasOwnProperty(element)) { // eslint-disable-line
                ...
                for (var element in instance.elements) { // eslint-disable-line
                    if (instance.elements.hasOwnProperty(element)) { // eslint-disable-line
                        ...
                        for (var i = 0; i < ContactDataServices.defaults.addressLineLabels.length; i++) {
                            ...
                            if (label === element && instance.elements[element] && instance.elements[element] !== instance.elements.input) {
            ...
            updateAddressLine: function (key, addressLineObject, className) {
                ...
                if (instance.elements[key]) {
                    ...
                    if (addressField.nodeName === 'INPUT' || addressField.nodeName === 'TEXTAREA' || addressField.nodeName === 'SELECT') {
                        if (addressField.nodeName === 'SELECT' && addressField.form == null) {
                            $('#billingState').val(value);
                        } else {
                            addressField.value = value;
                        }
                        if (addressField.nodeName === 'SELECT') {
                            var fieldId = addressField.id;
                            if (fieldId.includes('State')) {
                                if ($('#checkout-main').attr('data-checkout-stage') === 'shipping') {
                                    $('select[name $= "addressFields_states_stateCode"]#shippingStatedefault').val(value).trigger('change');
                                } else {
                                    $('select[name $= "addressFields_states_stateCode"]#billingState').val(value).trigger('change');
                                }
                            }
                        }
                    } else {
                    ...
                    // Remove the error messages when the value is filled
                    $(addressField).closest('.form-group').removeClass('error-field');
        ...
        instance.toggleVisibility = function (scope) {
            scope = scope || document; // eslint-disable-line
        ...
        if (($('#qasShipping').val() === 'false') || ($('#qasBilling').val() === 'false')) {
            instance.init();
        }
    ...
    ContactDataServices.address(options);
}
