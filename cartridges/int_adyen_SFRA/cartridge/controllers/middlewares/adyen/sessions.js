...
var paymentMethodNames = require('*/cartridge/adyenConstants/paymentMethodNames');
...
function getConnectedTerminals() {
  var adyenPosPaymentMethod = PaymentMgr.getPaymentMethod(constants.METHOD_ADYEN_POS);
  if (adyenPosPaymentMethod && adyenPosPaymentMethod.isActive()) {
...
function callCreateSession(req, res, next) {
  try {
    ...
    if (!response) {
      ...
      res.json({
          errorMessage: 'Failed to create Adyen Checkout Session',
          error: true
      });
      return next();
    }
