...
function handlePayment(stateData, order, options) {
  ...
  var orderNo = options.req.form.merchantReference;
  var paymentInstruments = currentBasket.getPaymentInstruments(constants.METHOD_ADYEN_COMPONENT);
  ...
  stateData.orderNo = orderNo;
  Transaction.wrap(function () {
    adyenPaymentInstrument.custom.adyenStateData = JSON.stringify(stateData);
  });

  res.redirect(URLUtils.url('Checkout-Begin', 'stage', 'placeOrder', 'isPayPal', true));
  return options.next();
