...
function getLabel(isStored, paymentMethod, paymentMethodName) {
  // Exception for Dotpay / Local Polish Payment Methods
  if (constants.METHOD_DOTPAY && constants.METHOD_DOTPAY.toLocaleLowerCase() === paymentMethod.type) {
    paymentMethodName = constants.METHOD_DOTPAY;
  }

  var label = isStored ? " ".concat(store.MASKED_CC_PREFIX).concat(paymentMethod.lastFour) : '';
  return "".concat(paymentMethodName || paymentMethod.name).concat(label);
}
...
function getListContents(_ref2) {
  ...
  var paymentMethodName = _ref2.paymentMethodName;
  var label = getLabel(isStored, paymentMethod, paymentMethodName);
  var liContents = "\n <div class=\"ayen-payWrapper\"><input name=\"brandCode\" type=\"radio\" value=\"".concat(paymentMethodID, "\" id=\"rb_").concat(paymentMethodID, "\">\n <label id=\"lb_").concat(paymentMethodID, "\" for=\"rb_").concat(paymentMethodID, "\">").concat(label, "</label>\n <div class=\"pay-image-wrapper\"><img class=\"paymentMethod_img\" src=\"").concat(imagePath, "\" ></img></div>\n <div class=\"clearfix\"></div> </div>\n ");
  return (description && paymentMethod.name !== 'PayPal') ? "".concat(liContents, "<p>").concat(description, "</p>") : liContents;
}
...
module.exports.renderPaymentMethod = function renderPaymentMethod(paymentMethod, isStored, path) {
  ...
  var paymentMethodName = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : null;
  var rerender = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : false;
  ...
  if (rerender) {
    ...
    li.setAttribute("class", paymentMethodID);
