...
function getPaypalConfig() {
  ...
  return {
    ...
    onClick: function onClick(data, actions) {
      ...
      $('#selectedPaymentOption').val('AdyenComponent');
...
function removeGiftCard() {
  $.ajax({
    type: 'POST',
    url: $('.b-checkout_page').data('cancel-partial-payment-order-url'),
...
function getGiftCardConfig() {
  ...
  return {
    ...
    onBalanceCheck: function onBalanceCheck(resolve, reject, requestData) {
      $.ajax({
        type: 'POST',
        url: $('.b-checkout_page').data('adyen-check-balance-url'),
    ...
    onOrderRequest: function onOrderRequest(resolve, reject, requestData) {
      ...
      $.ajax({
        type: 'POST',
        url: $('.b-checkout_page').data('adyen-partial-payments-order-url'),
        data: JSON.stringify(requestData),
        contentType: 'application/json; charset=utf-8',
        async: false,
        success: function success(data) {
...
function handleOnAdditionalDetails(state) {
  $.ajax({
    type: 'POST',
    url: $('.b-checkout_page').data('adyen-payment-details-url'),
    data: JSON.stringify({
      data: state.data,
