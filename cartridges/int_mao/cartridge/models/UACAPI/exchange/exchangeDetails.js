...
function ExchangeDetailsModel(originalOrder) {
    ...
    order.status = orderStatusMapping[originalOrder.rmaStatus] ? orderStatusMapping[originalOrder.rmaStatus] : originalOrder.rmaStatus;
    order.displayStatus = orderStatusModel[order.status] ? orderStatusModel[order.status] : order.status;
