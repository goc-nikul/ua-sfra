...
/**
 * build.js module adapted for Under Armour use from SFCC's community suite sgmf-scripts
 */
...
console.log(chalk.bgYellow.black.bold('Starting Under Armour Build Script v' + packageFile.uaversion + ' for SFRA v' + packageFile.version));
...
function getCartridges(packageFile) {
    ...
    for (let site of packageFile.sites) {
        for (let cartridge of site.cartridges) {
            ...
            if (cartridge.subPath) {
                cartridges.push(cartridge.subPath + "/" + cartridge.name);
            } else {
...
if (options.cover) {
    ...
    let unitFilePath = ' -- -R spec \'test/unit/**/*.js\'';
    if (os.platform() === 'win32') {
        unitFilePath = ' -- -R spec test/unit/**/*.js';
    }
    const subprocess = spawn(
        istanbul +
        ' cover ' +
        ...
        mocha +
        unitFilePath, { stdio: 'inherit', shell: true, cwd });
