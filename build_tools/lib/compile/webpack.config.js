...
module.exports = class WebpackConfigurator {
    ...
    createJsConfiguration() {
        return Object.assign(this.getCommonConfigurationOptions(), {
            ...
            optimization: {
                minimize: this.isOption('minimize') ? true : false,
            }
        });
    }
    ...
    getJsFiles() {
        ...
        for (const cartridge of this.cartridges) {
            if (cartridge.subPath){
                clientPath = path.resolve(this.cartridgesPath, cartridge.subPath, cartridge.name, 'cartridge/client');
            } else {
                var clientPath = path.resolve(this.cartridgesPath, cartridge.name, 'cartridge/client');
            }
    ...
    getResolver(type) {
        ...
        for (const cartridge of this.cartridges) {
            if (cartridge.subPath){
                var clientPath = path.resolve(this.cartridgesPath, cartridge.subPath, cartridge.name, 'cartridge/client');
            } else {
                var clientPath = path.resolve(this.cartridgesPath, cartridge.name, 'cartridge/client');
            }
