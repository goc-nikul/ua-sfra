...
var originalOrder = {};
describe('int_mao/cartridge/models/UACAPI/exchange/exchangeDetails.js', () => {
    let ExchangeDetailsModel = proxyquire('../../../../cartridges/int_mao/cartridge/models/UACAPI/exchange/exchangeDetails.js', {
    ...
    it('should throw an error when returnexchange model orderItems collection is empty', () => {
        originalOrder.exchangeOrder.orderItems = [];
        var result = new ExchangeDetailsModel(originalOrder);
        assert.isNotNull(new ExchangeDetailsModel(originalOrder), [], 'Order Object should throw an error');
        assert.isArray(result.orderItems, [], 'should return empty array');
    });

    it('should throw an error when returnexchange model orderItems collection is empty', () => {
        originalOrder.returnItems = [{
            orderNo: '12345',
            creationDate: '',
            shipments: [{}],
            orderItem: {
        ...
        var result = new ExchangeDetailsModel(originalOrder);
        assert.isNotNull(result, 'ExchangeDetailsModel not null');
        assert.isObject(result, 'Returns order');
        assert.isArray(result.orderItems, [], 'should return empty array');
    });

    it('should throw an error when returnexchange model orderItems collection is empty', () => {
        originalOrder.exchangeOrder.orderNo = '';
        var result = new ExchangeDetailsModel(originalOrder);
        assert.isNotNull(result, 'Order Model shouldnot null');
        assert.isObject(result, 'Order Model should be object');
        assert.equal(result.orderNo, '', 'Order Model should be object');
    });
