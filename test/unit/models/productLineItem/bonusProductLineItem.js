...
describe('Bonus Order Line Item', function () {
    ...
    var bonusProductLineItem = proxyquire('../../../../cartridges/app_ua_core/cartridge/models/productLineItem/bonusProductLineItem', {
    ...
    it('should call base for bonus Product item model (order)', function () {
        bonusProductLineItem(object, productMock, optionsMock);

        assert.isTrue(productDecorators.stubs.stubBase.calledOnce);
    });

    it('should call images for bonus Product item model (order)', function () {
        bonusProductLineItem(object, productMock, optionsMock);

        assert.isTrue(productDecorators.stubs.stubImages.calledOnce);
    });

    it('should call variationAttributes for bonus Product item model (order)', function () {
        bonusProductLineItem(object, productMock, optionsMock);

        assert.isTrue(productDecorators.stubs.stubVariationAttributes.calledOnce);
    });

    it('should not call price for bonus Product item model  (order)', function () {
        bonusProductLineItem(object, productMock, optionsMock);

        assert.isFalse(productDecorators.stubs.stubPrice.calledOnce);
    });

    it('should call quantity for bonus Product item model (order)', function () {
        bonusProductLineItem(object, productMock, optionsMock);

        assert.isFalse(productLineItemDecorators.stubs.stubQuantity.calledOnce);
    });

    it('should call uuid for bonus Product item model (order)', function () {
        bonusProductLineItem(object, productMock, optionsMock);

        assert.isTrue(productLineItemDecorators.stubs.stubUuid.calledOnce);
    });

    it('should call orderable for bonus Product item model (order)', function () {
        bonusProductLineItem(object, productMock, optionsMock);
    ...
    it('should call shipment for bonus Product item model (order)', function () {
        bonusProductLineItem(object, productMock, optionsMock);

        assert.isTrue(productLineItemDecorators.stubs.stubShipment.calledOnce);
    });

    it('should call priceTotal for bonus Product item model (order)', function () {
        bonusProductLineItem(object, productMock, optionsMock);

        assert.isTrue(productLineItemDecorators.stubs.stubPriceTotal.calledOnce);
    });

    it('should call options for bonus Product item model (order)', function () {
        bonusProductLineItem(object, productMock, optionsMock);

        assert.isFalse(productLineItemDecorators.stubs.stubOptions.calledOnce);
    });

    it('should call bonusProductLineItemUUID for bonus Product item model (order)', function () {
        bonusProductLineItem(object, productMock, optionsMock);

        assert.isFalse(productLineItemDecorators.stubs.stubBonusProductLineItemUUID.calledOnce);
    });

    it('should call preOrderUUID for bonus Product item model (order)', function () {
        bonusProductLineItem(object, productMock, optionsMock);
